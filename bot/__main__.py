...
async def set_commands(dp: Dispatcher):
    ...
    bot_commands = [
        ...
        BotCommand(command="generate_low", description="Random 16 chars password length  with letters, digits, separators"),
        BotCommand(command="generate_normal", description="Random 24 chars password length  with letters, digits, separators"),
        BotCommand(command="generate_hard", description="Random 32 chars password length  with letters, digits, separators"),
        BotCommand(command="generate_weak", description="2 words, random uppercase, separated by digits"),
        BotCommand(command="generate_medium", description="3 words, random uppercase, separated by digits"),
...
async def main():
    ...
    xkcd = XKCD(str(settings.chars.wordfile))
