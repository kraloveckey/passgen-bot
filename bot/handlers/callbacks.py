...
async def update_settings_message(call: types.CallbackQuery, data: dict):
    ...
    separators_enabled = data.get("separators", settings.chars.separators_by_default)
    prefixes_enabled = data.get("prefixes_suffixes", settings.chars.prefixes_suffixes_by_default)
    new_settings_text = get_settings_string(lang_code, chars_count, separators_enabled, prefixes_enabled)
    new_keyboard = make_settings_keyboard(settings, lang_code, chars_count, separators_enabled, prefixes_enabled)
...
async def change_chars_count(call: types.CallbackQuery, callback_data: dict, state: FSMContext):
    """
    Change number of chars to generate in password
    ...
    """
    data = await state.get_data()
    old_value = data.get("chars_count", 3)

    if callback_data["change"] == "plus" and old_value < settings.chars.max:
        await state.update_data(chars_count=old_value + 1)
    elif callback_data["change"] == "minus" and old_value > settings.chars.min:
        await state.update_data(chars_count=old_value - 1)
...
def register_callbacks(dp: Dispatcher):
    ...
    dp.register_callback_query_handler(change_chars_count, cb_charcount.filter(change=["minus", "plus"]))
